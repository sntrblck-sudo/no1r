...
for attempt in range(MAX_TRADES_DAY):
    ...
    if confidence < MIN_CONF:
        day_record['trades'].append({'action':'skip_low_conf','confidence':round(confidence,2)})
        continue
    ...
    if action=='buy' and amount>0:
        ...
        note='hold'
    day_record['trades'].append({'action':note,'amount':amount,'cost':cost,'revenue':revenue,'confidence':round(confidence,2),'balance':round(state['balance_sim'],2)})
    trades_executed += 1
